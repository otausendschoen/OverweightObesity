...
```{r}
#add past ubanization
urban_past<-read.csv("1975.csv", skip = 1, header = T)

urban_past2<-read.csv("2009.csv", skip=1, header = T)

urban_past<-merge(urban_past[,-c(2,3)], urban_past2[,-3], by="ISO.3166.1.numeric.code")
urban_past<-urban_past[, -1]
urban <- pivot_longer(urban_past, cols = -Location, names_to = "Year", values_to = "UrbanPopulationPercentage")
urban$Year <- gsub("X", "", urban$Year)
urban$Year <- gsub(".x", "", urban$Year)
urban$Year <- gsub(".y", "", urban$Year)
urban$Year<-as.numeric(urban$Year)
urban$Location <- trimws(urban$Location)
urban<-unique(urban)


length(unique((urban$Year)))
length(unique(urban_per))
```
```{r}
library(forecast)
...
for (country in unique(urban_per$Location)) {
  ...
  if (!is.null(arimax_model)) {
    ...
    forecast_df_country <- data.frame(Country = rep(country, 32), Year = seq(2019, 2050), Forecast_men = as.numeric(forecasts_original_scale))
...
for (country in unique(urban_per$Location)) {
  ...
  xreg_data <- cbind(urban_df$UrbanPopulationPercentage, ag_popu_df$PopFemale)
  xreg_data2 <- cbind(urban_df2$UrbanPopulationPercentage, ag_popu_df2$PopFemale)
  ...
  if (!is.null(arimax_model)) {
    ...
    # Create a data frame for forecasted values
    forecast_df_country <- data.frame(Country = rep(country, 32), Year = seq(2019, 2050), Forecast_women = as.numeric(forecasts_original_scale))
...
for (country in unique(urban_per$Location)) {
  ...
  if (!is.null(arimax_model)) {
    ...
    # Create a data frame for forecasted values
    forecast_df_country <- data.frame(Country = rep(country, 32), Year = seq(2019, 2050), Forecast_boys = as.numeric(forecasts_original_scale))
...
for (country in unique(urban_per$Location)) {
  ...
  if (!is.null(arimax_model)) {
    ...
    # Create a data frame for forecasted values
    forecast_df_country <- data.frame(Country = rep(country, 32), Year = seq(2019, 2050), Forecast_girls = as.numeric(forecasts_original_scale))
...
projection_data <- data.frame(
  ...
  Source = c("NCD-Risk Factor Collaboration (1975-2016)", "United Nations Population Division (UNPD)", "OECD Health Policy Studies", "Institute for Health Metrics and Evaluation", "Okunogbe A. et al.", "International Energy Agency", "World Bank - World Development Indicators", "World Bank - World Development Indicators", "Espallardo, Olga et al.", "Kudel et al.", "United Nations Population Division (UNPD)", "IHME - Global Burden of Disease Study", "IHME - Global Burden of Disease Study", "World bank - World Development Indicators", "World bank - World Development Indicators", "ILO global wage report", "Kudel et al.", "Espallardo, Olga et al.", "Kudel et al.", "DiBonaventura et al."),
...
```{r st, eval = T, include = T, fig.cap = "Determining the Order of the ARIMA Model"}
library(forecast)
library(tseries)
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
...
In Table \@ref(tab:forecasts), we can see some overweight and obesity projections in the columns "Forecast *X*" with *X* being the corresponding group. These forecasts are expressed as relative values.

\hspace{-2.5em}


```{r forecasts, eval = T, include = T}
...
Similar to the resulting model by *Okunogbe A. et al.*, there is a significant positive association between OAF and obesity prevalence at a significance level of 5%. (\( \beta =0.09531, p = 0.00283 \))(Figure \@ref(fig:Regression))
...
sampleprojection<-(predictions_oao[c(5,1,27, 296, 302,309),c(1,4,7, 19) ])
...
- \(\text{Gasoline price}\): The nation-specific price of gasoline per liter.
...
sampleprojection<-(predictions_oao[c(5,1,27, 296, 302,309),c(1,4,23) ])
...
sampleprojection<-(predictions_oao[c(5,1,27, 296, 302,309),c(1,4,26)])
...
In regards to the subsequent sections, it is furthermore important to note that we assume the same employment rates by BMI status. Although it is conceivable that people who are obese may have lower employment rates than people with a healthy weight, the available data is conflicting and equivocal [@Okunogbee006351].
...
\[ \text{YLLs due to Overweight and Obesity} = \sum_{c} \left(\text{PAF}_{\text{c}} \times \text{Total YLLs}_{\text{c}}\right) \]
...
colnames<-c("Country", "Year", "Value of a life year", "YLLs", "OAO YLLs", "Premature mortality Cost in Billions")
test1<-predictions_oao[employment_income_predictions$year==2019 & employment_income_predictions$Country.Name %in% c("Austria", "Australia", "United Arab Emirates", "Brazil"), ]

test2<-ylls_all_sum[ylls_all_sum$location_name %in% c("Austria", "Australia", "United Arab Emirates", "Brazil"), c(4,5,6,7)]
sampleprojection<-merge(test1[, c(1,2,29)], test2, by.x= "Country.Name", by.y= "location_name")
sampleprojection<-sampleprojection[, c(1,4,2,5,6,3)]
sampleprojection$premature_mort_costs<-sampleprojection$premature_mort_costs/10^9
...
sample <- employment_income_predictions[c(296, 321, 5, 4997), c(1, 26, 33, 36, 38)]
...
sample <- employment_income_predictions[c(296, 321, 5, 4997),  c(1, 26, 33, 36, 38, 41)]
...
sample<-employment_income_predictions[employment_income_predictions$Country.Name %in% interest & employment_income_predictions$year==2019, c(1, 26, 33, 36, 38,41)]
#sample<-employment_income_predictions[employment_income_predictions$Country.Name %in% interest & employment_income_predictions$year==2019, c(3, 27, 34, 37, 39,42)]
...
# Define the data
colnames <- c("Country", "Direct medical Costs", "Direct non-medical Costs", "Premature mortality Costs", "Absenteeism Costs", "Presenteeism Costs")
sample<-employment_income_predictions[employment_income_predictions$Country.Name %in% interest & employment_income_predictions$year==2050, c(1, 26, 33, 36, 38,41)]
#sample<-employment_income_predictions[employment_income_predictions$Country.Name %in% interest & employment_income_predictions$year==2019, c(3, 27, 34, 37, 39,42)]
...
```{r, eval = T, include = T, fig.cap="Cost Development in % of GDP"}
...
As highlighted by *Okunogbe et al.* even a modest reduction of overweight and obesity would save massive amounts of money - if current trends of increases in overweight and obesity continue. By reducing overweight and obesity prevalence by 5% each year, all cost parameters will be effected and total costs will be considerably lower in the end. A reduction of 5% of the projected overweight and obesity would result into total cost reductions of 2456 billion USD within the years 2019-2050.
...
##  Adult and childhood/adolescent overweight and obesity prevalence, by sex, 2019, 2024 and 2050
...
years_of_interest <- c(2019, 2024, 2050)
sampleprojection <- employment_income_predictions[employment_income_predictions$year %in% years_of_interest & employment_income_predictions$Country.Name %in% Country, c(3,12,27,34,37,39,42, 43)]
