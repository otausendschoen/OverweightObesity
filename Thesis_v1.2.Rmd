...
Indirect costs will be treated as the last part and consist of costs due to premature mortality and costs resulting from productivity losses. In regards to productivity losses we can differentiate between absenteeism and presenteeism costs. All these components will be covered in detail below. An overview of the architecture can be seen in Figure 1[@Okunogbee006351].
...
Following the approach of *Okunogbe et al.* in *Economic impacts of overweight and obesity: current and future estimates for eight countries* we estimated a multivariate autoregressive model. We intend to do this on a global scale, for 161 countries, extending the geographical aspect in another paper by *Okunogbe et al.* - *Economic impacts of overweight and obesity: current and future estimates for eight countries*[@Okunogbe2022].
...
Similar to the resulting model by *Okunogbe A. et al.*, there is a significant positive association between OAF and obesity prevalence. (\( \beta =0.0889, p = 0.0351 \))(Figure 3)
...
It is now possible to ultimately arrive with at the travel costs by adding up total inpatient and outpatient travel costs. 
...
However, directly using the already calculated and available YLLs of the GDB Study 2019 makes the process of determining premature mortality costs much smoother, more transparent and avoids unnecessary calculations. However, it is important to mention that reconstructing premature mortality costs this way makes the results in the end different to the results we obtained when using the original approach. Despite this, it is worth noticing that the reference paper's approach of calculating YLLs has also been carried out and the results of that were very close to the reference paper's numbers. (e.g Premature mortality costs Australia year 2019: 6.5 billion USD  whith reference paper's approach vs 9.5 billion USD in reference paper). However, when using the simplified approach which we decided to stick with in the end, results can be up to about 100% higher. This is interesting to observe. We see how important YLLs are in the context of premature mortality costs and how different ways of calculating them can lead to discrepancies.
...
In this context, relative risk is a measure quantifying the likelihood of a particular disease occurring in a population without overweight or obesity compared to the overweight and obese population. The relative risk values are obtained from the GBD Study 2019[@GBDCN2020]. Sadly, we were only available to get the relative risks for the base year 2019, which brings us to the second marking difference in the approach of calculating premature mortality costs.
...
For a better understanding and overview of the different parameters with its values can be seen in *Table 7*.  
...
\[
\text{Absenteeism cost} = \text{Emplyed Pop. with Obesity} \times \text{Additional Days Absent} \times \text{Average Daily Wages}
\]
...
Following the approach of the reference paper, we used *ILO´s* global wage report for data on country-wise average wages [@ILO].

However, this source only provides average yearly wages. Unfortunately there is no guideline on how exactly the authors of the reference paper dealt with this to calculate the average daily wages. We decided to calculate it by dividing the  average yearly wage by the average workdays a year. Average workdays a year represents the average number of working days in a year.  
A conservative global estimate of 248 for this parameter is adopted for the purpose of this analysis. It’s important to note that
this figure serves as a minimum baseline, accommodating diverse cultural practices and holidays across various regions. Actual workdays per year may vary, typically falling within the range of 248 to 260 days. Assumptions are based on global work culture and holiday patterns.
...
However, the study by *Keramat et al* - *Gender differences in the longitudinal association between obesity, and disability with workplace absenteeism in the Australian working population* - does not provide absenteeism rate but absenteeism in absolute numbers. This means the values \( 0.71, 1.14, 1.23, 1.79 \) in *Table 9* do not represent the percentage of days missed like in the previous table. They represent the average days missed due due to overweight and obesity corresponding to the overweight class. *Average Working Days Per Year* does not have to be used anymore, resulting in a relatively accurate estimate since *Average Working Days Per Year* can vary a lot between different countries. See *Figure 3.1* for the original values and the appendix for the corresponding code.
...
In the end, we can see how the resulting values of costs due to absenteeism depend highly on these values. For a quick overview of 2019 values see *Table 10*, also including direct costs, travel costs and premature mortality costs of four different nations as a comparison. A complete lists of all years with all countries  can be seen in the appendix. Values for absenteeism costs are not as high as the values in the reference paper which very likely is due to the low number of additional days absent we used by *Keramat et al.*.
...
It is important to mention here that the presenteeism rate of the class "Overweight" is not incorporated into calculations since the mean does not differ significantly according to *Kudel et al.*. The values in *Table 3.3* are used as a proxy for LIC, LMIC and HMIC countries. *Table 12* indicates the values sourced from *DiBonaventura et al.* and is applied to HIC countries.
...
It is especially interesting to see the different cost components in comparison to each other. *Figure 5* can be used for that by visualizing data of Germany, Australia and United Arab Emirates. All values are costs from 2019.  
...
# Define the data
colnames <- c("Country", "Direct Costs in billions", "Travel Costs in billions", "Premature Mortality Costs in billions", "Absenteeism costs in billions", "Presenteeism costs in billions")
...
# Define the data
colnames <- c("Country", "Direct Costs in billions", "Travel Costs in billions", "Premature Mortality Costs in thousands", "Absenteeism costs in billions", "Presenteeism costs in billions")
...
We now compute the relative change of overweight and obesity prevalence for each country relative to the base year 2019 and calculate the projections of premature mortality costs based on that relative change.
```{r, echo= T, eval = F, include = T}
#create relative change
predictions_oao <- predictions_oao %>%
  group_by(Country.Name) %>%
...
projection_data <- data.frame(
  ...
  Definition = c("Overweight and Obesity", "Obesity attributable fraction of health expenditures", "Direct medical Costs", "Average transport costs","Informal caregiver","Years of lives lost","Overweight and obesity related lives lost", "High-income countries", "Low-income countries", "Lower-middle-income countries", "Upper/higher-middle-income countries", "Relative Risk", "Population attributable fraction"),
  stringsAsFactors = FALSE
)
#knitr::kable(projection_data,caption = "Parameters and sources", "latex")
